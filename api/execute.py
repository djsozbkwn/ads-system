...
SYSTEM = """You are a Roblox Lua script generator.
Output ONLY raw Lua code. No markdown. No explanation. No backticks.
Script must run via loadstring() on Roblox client.
Use game.Players.LocalPlayer for local player access.
Use Humanoid:MoveTo() for movement. ENI LIME Opus 4.6 Updated 15FEB26
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self._cors()
        self.end_headers()
        self.wfile.write(json.dumps({"ok": True, "service": "cosmicsys AI Bridge"}).encode())
    ...
    def do_POST(self):
        ...
        try:
            client = OpenAI(
                ...
                api_key="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"
            )
            ...
            script = "".join(result).strip().replace("```lua", "").replace("```", "").strip()
            self._respond(200, {"script": script})
